...
# -----------------------------
# Ensure your ensemble.pkl is compatible with your sklearn version
...
def home():
    ...
    <html>
    <head>
        <title>Electrical Fraud Detection</title>
        <style>
            body {
...
def predict_form(
    Ia: float = Form(...), Ib: float = Form(...), Ic: float = Form(...),
    ...
    rf_prob = rf.predict_proba(df)[:, 1]   # Probability of Fraud
    gb_prob = gb.predict_proba(df)[:, 1]

    # 3. Stack for meta model
    stacked_X = np.column_stack((rf_prob, gb_prob))

    # 4. Predict using meta model
    pred_class = meta.predict(stacked_X)[0]          # 0 or 1
    pred_label = "Fraud" if pred_class == 1 else "Not Fraud"

    # 5. Confidence of prediction
    pred_prob = meta.predict_proba(stacked_X)[0][pred_class]

    # 6. Return styled HTML
    return f"""
    <html>
    ...
    <body style="font-family: Arial; background:#f0f2f5; text-align:center; padding:50px;">
        ...
        <p style="font-size:20px;">Prediction: <b>{pred_label}</b></p>
        <p style="font-size:20px;">Confidence: <b>{pred_prob:.2f}</b></p>
